#!/usr/bin/env python3
"""
Complete ACE workflow for Banking Topic Classification.
...
Usage:
    ...
    cd /path/to/ace
    python3 run_banking_workflow.py
...
def save_report(results_dir, results_summary, best_playbook, args, n_train, n_val, n_test):
    ...
    with open(report_path, 'w') as f:
        f.write("# ACE Banking Topic Classification - Run Report\n\n")
...
def run_ace_training(ace_system, train_samples, val_samples, test_samples, processor, results_dir):
    ...
    config = {
        'num_epochs': 1,
        'max_num_rounds': 2,  # Max reflection rounds per sample
        'curator_frequency': 2,  # Run curator every 2 steps
        'eval_steps': max(len(train_samples) // 4, 10),  # Evaluate 4 times during training
        'save_steps': max(len(train_samples) // 4, 10),
        'playbook_token_budget': 30000,
...
def main():
    parser = argparse.ArgumentParser(description='ACE Banking Workflow')
    ...
    parser.add_argument('--generator-model', type=str, default='claude-sonnet-4-5-20250514',
                        help='Model to use for generator agent')
    parser.add_argument('--reflector-model', type=str, default='claude-sonnet-4-5-20250514',
                        help='Model to use for reflector agent')
    parser.add_argument('--curator-model', type=str, default='claude-sonnet-4-5-20250514',
                        help='Model to use for curator agent')
    args = parser.parse_args()
    
    log("\n" + "#"*70)
    log("#" + " "*15 + "ACE BANKING WORKFLOW" + " "*25 + "#")
    log("#"*70)
    ...
    results_dir = os.path.join(os.path.dirname(__file__), "results", f"banking_{timestamp}")
